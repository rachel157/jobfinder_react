export default function TimelineTemplate({ data, title }) {
  ...
  const getSkillLevel = (skill) => {
    if (typeof skill === 'string') return 70
    if (skill?.proficiency_level) {
      const levels = { beginner: 30, intermediate: 60, advanced: 85, expert: 100 }
      return levels[skill.proficiency_level.toLowerCase()] || 70
    }
    if (skill?.level) {
      const levels = { beginner: 30, intermediate: 60, advanced: 85, expert: 100 }
      return levels[skill.level.toLowerCase()] || 70
    }
    return 70
  }
  ...
  return (
    <div className="cv-template cv-template--timeline">
      <style>{`
      ...
      <div className="template-sidebar">
        ...
        {personalInfo.headline && (
          <p style={{ fontSize: '13px', opacity: 0.95, textAlign: 'center', marginTop: '6px', marginBottom: '20px', lineHeight: '1.5', fontStyle: 'italic', fontWeight: 300 }}>
            {personalInfo.headline}
          </p>
        )}
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
              {personalInfo.email && (
                <div className="template-contact-item">
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ width: '12px', height: '12px' }}>
        ...
        {skills.length > 0 && (
          <div className="template-sidebar-section">
            ...
            {skills.map((skill, idx) => (
              <div key={skill.id || skill.skill_id || idx} className="template-skill-item">
                <div className="template-skill-name">{getSkillName(skill)}</div>
                <div className="template-skill-level">
                  <div className="template-skill-level-fill" style={{ width: `${getSkillLevel(skill)}%` }}></div>
                </div>
              </div>
            ))}
          </div>
            ...
            {languages.map((lang, idx) => (
              <div key={idx} className="template-language-item">
                ...
                {(lang.certificate || lang.certificate_url) && (
                  <div style={{ marginTop: 3, fontSize: '11px', color: '#cbd5e1' }}>
        ...
        {experiences.length > 0 && (
          <div className="template-section">
            ...
            {experiences.map((exp, idx) => (
              <div key={exp.id || idx} className="template-timeline-item">
                <div className="template-timeline-header">
                  <div>
                    <div className="template-timeline-title">{exp.job_title || exp.position || 'Position'}</div>
                    <div className="template-timeline-subtitle">{exp.company_name || 'Company'}</div>
                  </div>
                  <div className="template-timeline-date">
                    {formatDateRange(exp.start_date, exp.end_date, exp.is_current)}
                  </div>
                </div>
                {exp.description && (
                  <div className="template-timeline-description">{exp.description}</div>
                )}
              </div>
            ))}
          </div>
        ...
        {educations.length > 0 && (
          <div className="template-section">
            ...
            {educations.map((edu, idx) => (
              <div key={edu.id || idx} className="template-timeline-item">
                <div className="template-timeline-header">
                  <div>
                    <div className="template-timeline-title">
                      {edu.degree || 'Degree'}{edu.field_of_study ? ` - ${edu.field_of_study}` : ''}
                    </div>
                    <div className="template-timeline-subtitle">{edu.institution_name || edu.school_name || 'Institution'}</div>
                  </div>
                  <div className="template-timeline-date">
        ...
        {projects.length > 0 && (
          <div className="template-section">
            ...
            {projects.map((project, idx) => (
              <div key={idx} className="template-timeline-item">
                <div className="template-timeline-header">
                  <div>
                    <div className="template-timeline-title">{project.name || project.title || 'Project'}</div>
                    {project.role && (
                      <div className="template-timeline-subtitle" style={{ color: '#475569' }}>
                    ...
                    {project.url && (
                      <div className="template-timeline-subtitle">
                        <a href={project.url} target="_blank" rel="noopener noreferrer" style={{ color: '#3b82f6', textDecoration: 'none' }}>{project.url}</a>
                      </div>
                    )}
                    {Array.isArray(project.links) && project.links.length > 0 && (
                      <div className="template-timeline-subtitle" style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginTop: '4px' }}>
                ...
                {project.description && (
                  <div className="template-timeline-description" style={{ whiteSpace: 'pre-line' }}>
                    {project.description}
                  </div>
                )}
              </div>
        ...
        {certifications.length > 0 && (
          <div className="template-section">
            ...
            {certifications.map((cert, idx) => (
              <div key={cert.id || idx} className="template-card">
                <div className="template-card-title">{cert.name || 'Certification'}</div>
                <div className="template-card-subtitle">{cert.issuing_organization || cert.issuing_org || 'Issuer'}</div>
        ...
        {awards.length > 0 && (
          <div className="template-section">
            ...
            {awards.map((award, idx) => (
              <div key={award.id || idx} className="template-card">
                <div className="template-card-title">{award.title || 'Award'}</div>
                <div className="template-card-subtitle">{award.issuer || 'Issuer'}</div>
                <div className="template-card-date">{formatDate(award.date)}</div>
                {award.description && (
                  <div className="template-timeline-description" style={{ marginTop: '8px' }}>{award.description}</div>
                )}
              </div>
            ))}
          </div>
          ...
          <div className="template-section">
            ...
            {references.map((ref, idx) => (
              <div key={idx} className="template-card">
                <div className="template-card-title">{ref.name || 'Reference'}</div>
                <div className="template-card-subtitle">{ref.position || ref.title || ''} {ref.company ? `at ${ref.company}` : ''}</div>
                {ref.email && (
                  <div className="template-card-date">{ref.email}</div>
                )}
                {ref.phone && (
                  <div className="template-card-date">{ref.phone}</div>
                )}
              </div>
            ))}
          </div>
